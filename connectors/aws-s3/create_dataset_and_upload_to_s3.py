...
def create_dataset(file_name, num_rows, month, year):
    """Create a dataset"""
    with Progress() as progress:
        task = progress.add_task("[cyan]Creating dataset...", total=num_rows)

        # Generate dates from 2011 to the specified month and year
...
if __name__ == "__main__":
    ...
    month = int(input(f"Enter the end month (1-12): "))
    year = int(input(f"Enter the end year (default: 2023): ") or 2023)

    start_time = time.time()
    create_dataset(file_name=file_name, num_rows=num_rows, month=month, year=year)
    end_time = time.time()
