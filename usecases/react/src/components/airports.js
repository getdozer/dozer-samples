...
function Airports() {
  const { records, fields } = useDozerEndpoint('pickup');
      ...
      <Table sx={{ minWidth: 650 }} aria-label="simple table">
        ...
        <TableBody>
          {records?.map(r => (
            ...
            >
              {fields.map(f => <>
                {(f.getTyp() === Type.Point) && <TableCell key={`header-${f.getName()}`}>{r[f.getName()].getX()}, {r[f.getName()].getY()}</TableCell>}
                {(f.getTyp() !== Type.Point) && <TableCell key={`header-${f.getName()}`}>{r[f.getName()]}</TableCell>}
              </>)}
            </TableRow>
          ))}
        </TableBody>
